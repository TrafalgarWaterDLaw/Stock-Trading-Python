...
if not os.environ.get("API_KEY"):
    ...
    # pk_cf61c6c6dbc7461d8fb4619e93954e70!!!!!!!!!!
...
def buy():
    if request.method == "POST":
        ...
        try:
            ...
            return apology("Shares must be an intergers")
        ...
        item_total = item_price * shares

        if cash < item_total:
            return apology("Not enough cash!")
        else:
            db.execute("UPDATE users SET cash = ? WHERE id = ?", cash - item_total, user_id)
...
def quote():
    ...
    if request.method == "POST":
        ...
        return render_template("quoted.html", stock=stock, dollarFunc=usd)
    else:
...
def register():
    ...
    if(request.method == "POST"):
        ...
        if not username:
            ...
            return apology("Confirmation password required!", 403)
        ...
        try:
            ...
            return apology("Username has already benn used!", 403)

    else:
        return render_template("register.html")


# def user_info(information):
#    if not request.form.get("information"):
#        return apology(f"must provide {information}", 403)
...
def sell():
    ...
    if request.method == "POST":
        ...
        sharesO = db.execute("SELECT shares FROM transactions WHERE user_id = ? AND symbol = ? GROUP BY symbol",
